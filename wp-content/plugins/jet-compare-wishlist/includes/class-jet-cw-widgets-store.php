...
if ( ! class_exists( 'Jet_CW_Widgets_Store' ) ) {
	...
	class Jet_CW_Widgets_Store {
		...
		public function get_stored_widgets() {
			return ! empty( $_REQUEST['widgets_data'] ) ? $_REQUEST['widgets_data'] : [];
		}
		 ...
		 * Localize data.
		 *
		 * Enqueue scripts & setup localized data.
		 *
		 * @since 1.0.0
		 *
		 * @return void
		...
		public function localize_data() {
			...
			wp_enqueue_script(
				'jet-cw-frontend',
				jet_cw()->plugin_url( 'assets/js/dist/frontend.js' ),
				$deps,
				jet_cw()->get_version(),
				true
			);
			...
			wp_localize_script( 'jet-cw-frontend', 'JetCWSettings', $localized_data );
